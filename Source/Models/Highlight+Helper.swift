...
extension Highlight {
    ...
    public func persist(completion: Completion? = nil) {
        do {
            try BookStorage.instance?.save(withHighlight: self)
            completion?(nil)
        } catch let error as NSError {
            printLog("Error on persist highlight: \(error)")
            completion?(error)
        }
    }
    ...
    public func remove() {
        do {
            if !self.highlightId.isEmpty {
                try  BookStorage.instance?.remove(withHighlightId: self.highlightId)
            } else if !self.bookId.isEmpty {
                try  BookStorage.instance?.remove(withBookId: self.bookId)
            }
        } catch let error {
            printLog("Error on remove highlight: \(error)")
        }
    }
    ...
    public static func removeById(highlightId: String) {
        do {
            try  BookStorage.instance?.remove(withHighlightId: highlightId)
        } catch let error {
            printLog("Error on remove highlight: \(error)")
        }
    }
    ...
    public static func getById(highlightId: String) -> Highlight? {
        do {
            return try BookStorage.instance?.query(withHighlightId: highlightId)
        } catch let error {
            printLog("Error getting Highlight : \(error)")
        }
        return nil
    }
    ...
    public static func updateById(highlightId: String, type: HighlightStyle) {
        do {
            try BookStorage.instance?.updateType(withHighlightId: highlightId, type: type.rawValue)
        } catch let error {
            printLog("Error on updateById: \(error)")
        }
    }
    ...
    public static func allByBookId(bookId: String, andPage page: NSNumber? = nil) -> [Highlight] {
        do {
            let highlights = try BookStorage.instance?.query(withBookId: bookId, page: page?.intValue)
            return highlights ?? []
        } catch let error {
            printLog("Error on fetch all by book Id: \(error)")
            return []
        }
    }
    ...
    public static func all() -> [Highlight] {
        do {
            let highlights = try BookStorage.instance?.queryAll()
            return highlights ?? []
        } catch let error {
            printLog("Error on fetch all: \(error)")
            return []
        }
    }
}
...
extension Highlight {
    ...
    @discardableResult public static func removeFromHTMLById(withinPage page: FolioReaderPage?, highlightId: String) -> String? {
        ...
        if let removedId = currentPage.webView?.js("removeHighlightById('\(highlightId)')") {
            ...
            printLog("Error removing Highlight from page")
            return nil
        }
    }
